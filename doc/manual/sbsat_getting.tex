...
\item[$\bullet$] Tracer (From CMU benchmark examples - described in Section~\ref{ref-trace-section})
\item[$\bullet$] XOR (Each conjoint is an XOR of conjoints -
see Sections~\ref{xor-section},\ref{ref-xor-section})
\end{enumerate}}
...
Figure~\ref{exec-figure}.  In the first phase an input is read from an
input file.  The user must decide which input format to use and build
the input file accordingly.  There are three issues here: namely
choosing the type of input format, writing the input in a way that can
be exploited by elements of the remaining two phases, and keeping the
syntax correct.  Format types and syntax are described in
Sections~\ref{basics-section} and~\ref{format-section}.  Comments on
writing exploitable input may be found in
Section~\ref{exploit-section}.  In the second phase various levels of
preprocessing are applied to the input instance with the intention of
producing an internal logically equivalent\footnote{See
Page~\pageref{s-eq-page} for the definition of logically equivalent.}
set of constraints (in canonical form) that yields a smaller search
space through advanced and intelligent search heuristics and learning.
The user may control this phase using command line switches when
launching the program.  Details of the kinds of preprocessing
available and their effects are found in
Sections~\ref{preproc-opts-section} and~\ref{preproc-section} along
with examples of their use.  In the third phase the internal form
(that is, set of constraints in canonical form) is searched for a
solution.  The user must choose one of four ways to perform a search
and the search heuristic which is used to select unassigned variables
to be assigned values. Future versions will allow the user to define a
search heuristic and coordinating preprocessing elements.  Choices for
searching are:

{\baselineskip=6pt
\begin{enumerate}
\item[$\bullet$] Backtracking (Section~\ref{lemma-section})
\item[$\bullet$] BDD Walksat (an incomplete solver - Section~\ref{walksat-section})
\item[$\bullet$] WMV (Vanfleet's tinkering solver - Section~\ref{wmv-section})
\end{enumerate}}

\noindent
Reasons for choosing one of the above are given in
Sections~\ref{lemma-section}-\ref{wmv-section}.  Search heuristics
are used to help control the size of the search space. In the current
version the user may choose one of the following:

{\baselineskip=6pt
\begin{enumerate}
\item[$\bullet$] Chaff-like (Section~\ref{chaff-section})
...
The size of the search space can be further controlled through
learning.  As backtracks occur, new constraints, called Lemmas
(described in Section~\ref{lemma-section}), are added to the internal
constraint set. These can prevent some fruitless backtracking later in
the search.  However, there is some overhead incurred by Lemmas.
Hence it is important to choose carefully which Lemmas are to be
saved, how many Lemmas can be saved at a maximum, and which Lemmas to
discard when the maximum is exceeded.  These choices are controlled by
switches on the command line and are described in
Section~\ref{ref-command-line-section}.  The results of operations
initiated by those switches are explained in Section~\ref{lemma-section}.
...
The solver described here does not have a GUI based interface; rather it
uses a command line interface.  However, a Java front end has been
developed and is capable of parallelizing solver computations.  That
GUI is to be described in a companion document.
...
arbitrarily many items of the type preceeding the ellipsis are
...
\indent{\tt [-]<var> ...}\\
\vspace*{-3mm}\\ 
means at least one {\tt <var>} item and each may or may not be
...
command line.  Switches are always preceeded by a dash ({\tt -}) or a
double dash ({\tt -{}-}).  All switches understood by {\tt sbsat} are
listed and described briefly in Section~\ref{ref-command-line-section}.

\item {\bf Boolean Variable} - A variable may or may not be assigned a
value: if it is assigned a value that value is one of the atoms in the
set $\{T,F\}$, where $T$ and $F$ may be thought of as corresponding to
{\em true} and {\em false}, respectively.  In this document we use
alternatively and interchangeably use the set $\{1,0\}$ for $\{T,F\}$
since so much of the literature uses that notation.  Hereafter, when we
say variable we mean Boolean variable.

\item {\bf Boolean Function} - A Boolean function has one or more
variable or Boolean function arguments and may or may not return a
Boolean value depending on values assigned to or returned from
its arguments.  Any Boolean function can be expressed in terms of a
nesting of Boolean functions given in the following definition.
This fact is used to express arbitrary Boolean functions in our
canonical form (see Section~\ref{ref-can-section}).

\item {\bf Unary, Binary, and Ternary Boolean Functions} ({\tt or},
{\tt nor}, %{\tt limp}, 
{\tt imp}, {\tt nimp}, {\tt xor}, {\tt and},
{\tt nand}, {\tt equ}, {\tt ite}, {\tt not}) - A Boolean function of
two variables.  There are $2^{2^2}=16$ different binary Boolean
functions and 2 unary functions.  Names associated with a subset of
these that include only non-trivial functions are given in the
following table where, for binary functions, the bits of the 1-0
strings correspond to function values given input values of {\tt 00},
{\tt 01}, {\tt 10}, and {\tt 11}, respectively, from left to right,
and for the unary function the two bit strings correspond to input
values of {\tt 0} and {\tt 1}, respectively, from left to right.  An
important ternary function is {\tt if-then-else} which we call {\tt
ite}.  Its functionality is also expressed in the table with the
obvious correspondence between input values and function values.
\begin{center}
\begin{tabular}{rlcrlcrl}
\multicolumn{5}{c}{\underline{\bf ~~~~~~~~~~~~~Binary~~~~~~~~~~~~~}} &
\multicolumn{1}{c}{} &
\multicolumn{2}{c}{\underline{\bf ~~~Unary~~~}}\\
{\tt or}  &{\tt 0111}&~~~~~&{\tt imp}  &{\tt 1101}&~~~~~&{\tt not}&{\tt 10}\\
{\tt nor} &{\tt 1000}&     &{\tt nimp} &{\tt 0010}&     &         &        \\
{\tt and} &{\tt 0001}&     &{\tt equ}  &{\tt 1001}&     & 
\multicolumn{2}{c}{\underline{\bf ~~~Ternary~~~}} \\
{\tt nand}&{\tt 1110}&     &{\tt xor}  &{\tt 0110}&     &
{\tt ite}&{\tt 01010011}\\
\end{tabular}
\end{center}
...
of a Boolean function expressed using only the operator {\tt
if-then-else}, plus constants {\em T} and {\em F}, Boolean variables, 
and parentheses.   BDD representations are usually far more compact
than truth table representations.  The form of BDDs we used are
reduced ordered as these are canonical representations of
functions.\label{bdd-definition-page}
...
function to another such that the target function is satisfiable if
and only if the source function is satisfiable is said to produce a
logically equivalent target function.\label{s-eq-page}
...
\item {\bf Choice point} - The point in a search where a variable is
given a value decided upon by some heuristic and not inferred.
...
\item {'{\tt\bf *}'} (star at the begining of an input file line) - 
The star is followed by a function with arguments.  Function lines
have the following syntax:
\begin{verbatim}
   *<function-identifier> (arg_1, arg_2, ..., arg_n)
\end{verbatim}
where {\tt arg\_}{\em i} may be a non-zero integer, referring to a
positive or negative literal, or a variable name, or {\tt \$<number>} 
where {\tt <number>} is the {\tt <number>}rd function line from the
top.  See Section~\ref{ref-can-section} for further details.
\end{itemize}
...
system but at least 200MB of free space is required.
...
take advantage of multiple processors\footnote{However, a Java tool
has been developed which can parallelize execution of {\tt sbsat}.}.


\subsection{Getting SBSAT}

SBSAT may be obtained by email request to either {\tt
...
transfer the file using zmodem.  All others will either receive the
tarball as an email attachment or the distribution CDROM, whichever is
chosen by the requestor.
...
Become {\tt root}.  This entails knowing the superuser password.  At
the command line prompt, issue the command {\tt su} and enter the
superuser password when requested to do so.
...
revision you have installed: for example, on August 25, 2003 the
version is {\tt 2.0r} and the revision is {\tt 1} so in this case
the directory is\\
\vspace*{-3mm}\\
\indent{\tt /usr/local/sbsat-2.0r-1}\\
\vspace*{-3mm}\\
...
\vspace*{-3mm}\\
\indent{\tt \$ ln -s sbsat-2.0r-1 sbsat}

\subsection{Compiling SBSAT}\label{compile-section}

Become {\tt root} as in Section~\ref{install-section}.  Change to the
directory containing the SBSAT files, called the {\em root directory
of the SBSAT tree}.  If you followed the instructions in
...
\indent{\tt \$ make}\\
\vspace*{-3mm}\\
From now on files and directories contained in or below the root
directory of the SBSAT tree will be referred to with {\tt .../}
prepended to their paths originating from that directory.  If no
errors are reported the SBSAT executable, named {\tt sbsat}, exists in
directory {\tt .../src}.  To use the executable conveniently from any
directory it is advisable to set a link to it from some directory that
is in your {\tt PATH}.  For example, if {\tt /usr/local/bin} is in
your {\tt PATH} (it normally is), as {\tt root} change directory to
{\tt /usr/local/bin} then set a link as in the following\\
\vspace*{-3mm}\\
...
-{}-help}.  The following two can be very useful.  
...
As {\tt root}, change to the {\tt tests} directory in the SBSAT
...
parameter without a dash is the output file.
...
\begin{figure}
\begin{verbatim}
   warning: ini file not found /home/fett/sbsat.ini
...
solver to output the solution.  The following command does this:\\
\vspace*{-3mm}\\
...
set a format is not necessary.  However, the switch {\it -{}-formatin
c} exists to force reading a cnf file (this switch is not listed when
displaying command-line help and should be considered deprecated).
...
\begin{verbatim}
   ...
   Total Time: 0.020
\end{verbatim}
\caption{Output generated by command {\tt \$ sbsat -R r small.cnf}.}
...
\noindent
\textbf{\underbar{Remark}}: ~A dash ({\tt -}) in place of
the filename denotes the standard input or standard output
depending on the context.  For example, execution of the following
command should be followed by keyboard input representing some
accepted input format.\\
\vspace*{-3mm}\\
\indent{\tt \$ sbsat small.cnf} \emph{-R} r {\tt -}\\
\vspace*{-3mm}\\
All available switches can be printed using the following command:\\
\vspace*{-3mm}\\
\indent{\tt \$ sbsat} \texttt{\emph{-{}-help}}\\
\vspace*{-3mm}\\
which gives an output shown, in part, in Figure~\ref{help-figure}.
...
Section~\ref{ref-can-section}.  Canonical form depends on the notion
of BDDs which is explained in Section~\ref{bdd-section}.
...
\begin{verbatim}
   warning: ini file not found /home/fett/sbsat.ini
   Reading File ../examples/xortest.xor ....
   Reading XOR ... Done
   Preprocessing .... Done
   Creating Smurfs ... Done
   Solution verified.
   ...
   0.000 backtracks per sec.
   Time: 0.000s. Backtracks: 0 (10000000.000 per sec) Progress:  0.00%
    Choices (total, dependent, backjumped): (6, 0, 0)
    Lemmas (cached, non-cached, added): (0, 7, 7)
    Inferences by smurfs: 7; lemmas: 0;
    Backtracks by smurfs: 0; lemmas: 0;
    Backjumps: 0 (avg bj len: 0.0)
   Choice Points: 5 , Backtracks: 0, Backjumps: 0
   Satisfiable
   Total Time: 0.010
\end{verbatim}
...
\begin{figure}
\begin{verbatim}
p bdd 20 11 ; 20 variables, 11 functions
Initial_Branch(var*, a, b, t*e) ; These variables will be branched on first.
                                ; '*' is a wildcard

ite(var1, T, F)  ; Function 1
var1             ; Function 2 - make var1 true (assign it value T)
                 ; The two preceeding lines created identical functions
                 ; T for true and F for false is built-in
and(var1, var2)          ; Function 3
and4(a, b, 1, 2)         ; Function 4
*or3($3, not($4), -var1) ; Function 5 - use functions 3 and 4,
                         ; cannot use '-' before $4 instead of not
*ite 4 5 6               ; Function 6
                         ; Parentheses and commas are not required
ite 2                    ; Function 7
    ite 3 4 5    ; Comments are ignored, even in the middle of a function
    and or 6 7 8 ; Lines can be hard to read without parentheses and commas
...
; Macros consisting of previously defined functions
#define BBBM_MAJV(v1, v2, v3, v4)
# BBBM(majv(v1, v2, v3), majv(v1, v3, v4), majv(v1, v2, v4), majv(v2, v3, v4))
*bbbm_majv(tem1e, tem2e, tem3e, tem4e) ; Function 8
                                       ; There is no case sensitivity

; Overloading the AND operator to be the OR of 3 variables.
#define and(x, y, z) # or3(x, y, z)
*and(var1, var2, var3) ; Function 9
; Returning the AND operator to normal;
#define and(x, y) # ite(x, y, F)

print_tree  $7 ; No function created
pprint_tree $7 ; No function created
*minmax(4, 0, 1, x10, x9, x8, x7) ; Function 10
print_tree minmax(4, 0, 1, x10, x9, x8, x7) ; No function created
*or(pprint_tree(print_tree(and(2, 3))), 4) ; Function 11
...
*print_tree($11) ; No function created
...
\begin{center}
$
o_1\equiv ite(s_1, T, F)
\wedge o_2\equiv ite(s_3, o_6, o_5)
\wedge o_3\equiv ite(s_3, o_5, o_6)
\wedge o_4\equiv ite(s_2, T, T)
$
\\
$
\wedge o_5\equiv ite(s_1, F, T)
\wedge o_6\equiv ite(s_2, T, T)
$
\end{center}
These are conjoined with the constraints forcing {\em impulse}($1$) which
are
\begin{center}
$x_1\equiv F\wedge x_2\equiv T\wedge x_3\equiv T\wedge x_4\equiv T$
\end{center}
Similar constraints may be constructed for {\em impulse}($2$) to {\em
...
\begin{verbatim}
   p bdd 43 42
   equ(o11, s1)
   equ(o21, ite(s3, o61, o51))
   equ(o31, ite(s3, o51, o61))
   o41
   equ(o51, -s1)
   o61
   -x11
   ...
   x41
   equ(o12, -s1)
   equ(o22, ite(s3, o62, o52))
   equ(o32, ite(s3, o52, o62))
   o42
   equ(o52, s1)
   o62
   x12
   ...
   x42
   o13
   equ(o23, ite(s3, o63, o53))
   equ(o33, ite(s3, o53, o63))
   equ(o43, -s2)
   o53
   equ(o63, s2)
   x13
   ...
   x43
   o14
   equ(o24, ite(s3, o64, o54))
   equ(o34, ite(s3, o54, o64))
   equ(o44, s2)
   o54
   equ(o64, -s2)
   x14
...
Construct a Boolean expression which must have value {\em T} if and 
...
if the above query holds as follows (the conjunction of the following
three parts):
\begin{enumerate}
\item
...
v^3_2=1.\]
It may also be verified that no other assignment of values to $v^i_1$
and $v^i_2$, $0\leq i\leq 3$, satisfies the above expressions.  The
constraints are in canonical form in Figure~\ref{bmc-can-figure}.
...
%JVF - is -b -w the same as -bw?

By default, {\tt sbsat} searches using backtracking.  But
this can be changed using command line switches.  The table below
summarizes the switches and results.
\begin{center}
...
yes&
\texttt{\emph{-b}}&
Choice points, learning
\tabularnewline
\hline 
%Backtrack w/o Lemmas &
%no&
%\texttt{\emph{-b}}&
%Choice points, no learning
%\tabularnewline
%\hline 
BDD WalkSAT&
no&
\texttt{\emph{-w}}&
Random walk, not complete
\tabularnewline
\hline 
WVF search (experts)&
no&
\texttt{\emph{-m}}&
For keyboard input, experiments
\tabularnewline
\hline
\end{tabular}
\end{center}
...
be achieved by using {\em -In 0 -All 0} switches on the command line.

However, in some cases conversion is done so as to take advantage of
preprocessing.  Thus, given a file in {\tt xor} format, one could 
preprocess with a result in the same format or maybe even {\tt cnf}.

One might also want to convert between formats so that a problem might
be attempted on a variety of solvers.  For example, many problems come
in {\tt trace} or {\tt BDD} formats but Chaff does not recognize those
formats so they must be converted.

Format conversions supported by SBSAT are listed in the following
table.  To determine whether format {\tt A} can be converted to format
{\tt B}, locate {\tt A}'s row and the answer appears in {\tt B}'s
column.  For example, format {\tt dnf} converts to {\tt cnf} but not
from {\tt cnf} to {\tt dnf}.\\
...
\hline
{\bf cnf} & yes & no  & no  & no  & no & yes \\
{\bf dnf} & yes & yes & no  & no  & no & yes \\
{\bf xor} & yes & no  & no & no  & no & yes \\
{\bf bdd} & yes & no  & no  & no & no & yes \\
{\bf trace} & yes & no & no & no & no & yes \\
{\bf smurf} & yes & no & no & no & no & yes \\
\hline
\end{tabular}\\

\noindent
A sample command for translating a file of format to another is:\\
\vspace*{-3mm}\\
\indent{\tt \$ sbsat -c flowers.ite > flowers.cnf}
...
\vspace*{-3mm}\\
\indent{\tt \$ sbsat -c -All 0 -In 0 flowers.ite > flowers.cnf}
...
or\\
\vspace*{-3mm}\\
\indent{\tt \$ sbsat -c -All 0 -In 0 flowers.ite -{}-output-file flowers.cnf}
...
The many possible preprocessing
options available to {\tt sbsat} users are shown in the table below.
See Section~\ref{preproc-section} for an explanation of these
options and when an option might pay off and when it might be a
liability.\\

\hspace*{-5mm}\begin{tabular}{|c|c|c|c|c|}
\hline 
Name&Default&Option&Formats&Description\tabularnewline\hline\hline 
Clustering&yes&Cl&CNF& see Section~\ref{clustering-section}
\tabularnewline\hline 
Generalized Cofactoring&yes&Co&All& see Section~\ref{gcf-section}
\tabularnewline\hline 
Restrict&yes&Pr&All&see Section~\ref{branch-prune-section}
\tabularnewline\hline 
...
\tabularnewline\hline
Existentially Quantify And&yes&Ea&All&see Section~\ref{existential-all-section}
\tabularnewline\hline
...
Preprocessor options can be applied in any order, as desired, and
repeated by specifying the order and repetitions on the command line
in the {\em preprocessor sequence}.  A preprocessor sequence consists
of a list of {\em preprocessor run}s, or just {\em run}s.  Each run
except the last may be followed by a positive integer, {\tt *}, or
another run; the last run may be followed by a positive integer or
{\tt *}.  A run is a concatenation of preprocessor options from the
above table, wrapped inside matching parentheses.  The preprocessing
operations specified by the options of a run are applied in the order
in which they are specified.  The run may be repeated if it is
immediately followed by a positive integer $R$.  In that case the run
is repeated $R$ times or until the internal form is the same before
and after the run (that is, until reaching a fixed point).  If the run
is immediately followed by {\tt *} then it will repeat until reaching
a fixed point.  For example, the following is a run which applies
existential quantification, followed by dependent variable clustering:\\
\vspace*{-3mm}\\
\indent{\tt (ExDc)}\\
\vspace*{-3mm}\\
...
a preprocessor sequence wrapped in single quotes.  For example,\\
\vspace*{-3mm}\\
\indent
...
\indent 
{\tt \$ sbsat {\it -{}-output-file} newfile.smurf -s myoldfile}\\ 
...
The other search heuristic, which we call the {\em chaff-like heuristic},
is a variant of the VSIDS heuristic Chaff uses.  See
Section~\ref{chaff-section} for information about the use and control
of this heuristic.
...
Chaff-like &no& {\tt\it -H} {\tt l}&Number of occurrences of literals\tabularnewline\hline 
...
prevent any lemma from being stored beyond the point it is created.
But, lemmas are still created during backtracking and inferencing to
support backjumping.  To prevent the lemmas from being created
backjumping must be disabled {\em and} the lemma cache must
be set to 0.\footnote{See Section~\ref{lemma-section-cache} for
details.}  For example, use\\
\vspace*{-3mm}\\
\indent {\tt \$ sbsat {\it -L} 0 {\it -{}-backjumping} 0 slider\_80\_unsat.ite}\\
\vspace*{-3mm}\\ 
...
compatible with the option disabling lemma creation.  Also the
...
causes the lemmas not to be created ({\it -L 0 -{}-backjumping 0}).
...
%JVF - Section 3.3.2: If the preprocessor is cancelled after
...
When working on a problem that requires using a long command line over
and over, it is convenient to create an initialization file to prevent
having to reenter the switches on every run.  The initialization file,
called {\tt sbsat.ini}, contains a list of settings that are
translated to switches by {\tt sbsat} when it is invoked.  SBSAT
automatically looks for {\tt sbsat.ini} in the user's home directory
(that is, it looks for a file {\tt \~{}/sbsat.ini}).
...
%\noindent
%\textbf{\underbar{Remark}} ~ The name of the input file in the ini
%file (using \texttt{\footnotesize input-file=\char`\"{}small.cnf\char`\"{}})
%and start sbsat with the ini file only (using \texttt{\footnotesize \$
%sbsat} \texttt{\emph{\footnotesize -{}-ini}} \texttt{\footnotesize myini.ini}).
...
solved, and {\tt outputfile} names a file to which output from a run
of {\tt sbsat} can be directed.  The {\tt inputfile} can take several
formats, all somewhat different from each other, which are described in
Section~\ref{format-section}.  Options customize the execution of {\tt
sbsat}: they control preprocessing, search, input, and output
specifics and more.  Observe that {\tt options}, {\tt inputfile}, and
{\tt outputfile} are all optional.  

Options are invoked using switches.  A switch is preceeded by one or
two dashes ('{\tt -}' or '{\tt -{}-}) and may be immediately followed
by a parameter which is either an integer or a string, depending on
the switch.  If a switch requires a parameter, one or more blank
characters separates it from the parameter.  Switch/parameter pairs
are separated from each other and the file names by blanks.  There are
many switches and they are organized below by type.
...
\vspace*{-3mm}\\
\indent {\tt \$ sbsat -R r small.cnf}\\
\indent\indent\indent Solve the problem in {\tt small.cnf}, show the
result raw\\
\vspace*{-3mm}\\
\indent{\tt \$ sbsat -P '(ExDc)3(ExSt)2(ExPr)10' small.cnf}\\ 
...
\noindent
will invoke the walksat search (see Page~\pageref{walksat_page} for
options {\tt -b} and {\tt -w}).  Some switch combinations cooperate
with each other.  For example:\\
\vspace*{-3mm}\\
\indent{\tt \$ sbsat -All 0 -St 1}

\noindent
turns all preprocessing off then turns strengthening on (see
Page~\pageref{all_page} for preprocessing switches).

There are no known switch combinations that cause {\tt sbsat} to hang.
However, under certain circumstances {\tt sbsat} may be confused by
the switch combinations and exit gracefully.  For example:\\
\vspace*{-3mm}\\
\indent{\tt \$ sbsat -{}-backjumping 0}

\noindent
is an attempt at running lemmas without backjumping, which is
impossible.  The system responds with the following error message:\\
\vspace*{-3mm}\\
\indent{\tt error: Can not disable backjumping and have lemmas}\\
\indent{\tt ~~~~~Please set -L 0}

\noindent
In this case the user should write:\\
\vspace*{-3mm}\\
\indent{\tt \$ sbsat -L 0 -{}-backjumping 0}

\noindent
to backtrack without backjumping or\\
\vspace*{-3mm}\\
\indent{\tt \$ sbsat -L 0}

\noindent
to backtrack without lemmas.
...
\begin{tabular}{lcl}
\emph{-b} &-& Start default search {[}default{]}\\
\emph{-w} &-& Start walksat search\label{walksat_page}\\
\emph{-m} &-& Start WVF search\\
\emph{-n} &-& Don't start any search or conversion\\
...
\emph{-{}-tree-width <number>, -W <number>} &-& Set tree width
{[}default=64{]}\\
\emph{-{}-prover3-max-vars <number>} &-& Max variables per BDD when
reading\\
& & 3 address code (input format 3) {[}default=10{]}\\
\end{tabular}

\subsection{Trace format option}

\begin{tabular}{lcl}
\emph{-{}-module-dir <string>} &-& Directory to find extra modules {[}default=\char`\"{}./Modules\char`\"{}{]}\\
\end{tabular}

\subsection{LSGB heuristic options}
...
\begin{tabular}{lcl}
\emph{-{}-preprocess-sequence <string>,} &-&
The preprocessing sequence \\
\emph{~~~~~~~~~~~~~~~~~~~~~~-P <string>} & & {[}default=\char`\"{}(ExDc){*}(ExSt){*}(ExPr){*}\char`\"{}{]}\\
\emph{-{}-All, -All} &-& Enable/Disable All Preprocessing\label{all_page}\\
& & Options (1/0) \\
\emph{-{}-Cl <number>, -Cl <number>} &-& Enable/Disable Clustering (1/0)\\
& & {[}default=1{]}\\
\emph{-{}-Co <number>, -Co <number>} &-& Enable/Disable Cofactoring (1/0)\\
& & {[}default=1{]}\\
\emph{-{}-Pr <number>, -Pr <number>} &-& Enable/Disable Pruning (1/0)\\
& &  {[}default=1{]}\\
\emph{-{}-St <number>, -St <number>} &-& Enable/Disable Strengthening (1/0)\\
& & {[}default=1{]}\\
\emph{-{}-In <number>, -In <number>} &-& Enable/Disable Inferences (1/0)\\
& & {[}default=1{]}\\
\emph{-{}-Ex <number>, -Ex <number>} &-& Enable/Disable Existential
Quantification\\
& &  (1/0) {[}default=1{]}\\
\emph{-{}-Dc <number>, -Dc <number>} &-& Enable/Disable Dependent Variable\\
& & Clustering (1/0) {[}default=1{]}\\
\emph{-{}-Sp <number>, -Sp <number>} &-& Enable/Disable large function 
splitting\\
& & (1/0) {[}default=0{]}\\
\emph{-{}-max-preproc-time <number>} &-& Set the time limit in seconds (0=no
limit)\\
& & {[}default=0{]}\\
\emph{-{}-do-split-max-vars <number>} &-& Threshold above which the Sp\\
& & splits BDDs {[}default=10{]}\\ \label{split-page}
\end{tabular}

\subsection{Search options}

\begin{tabular}{lcl}
\emph{-{}-lemma-out-file <string>} &-& File to dump lemmas to
{[}default=''''{]}\\
\emph{-{}-lemma-in-file <string>} &-& File to read lemmas from 
{[}default=''''{]}\\
\emph{-{}-csv-trace-file <string>} &-& File to save execution trace in\\
& & CSV format {[}default=''''{]}\\
\emph{-{}-var-stat-file <string>} &-& File to save var stats
{[}default=''''{]}\\
\emph{-{}-brancher-presets <string>} &-& Variables that are preset
before brancher is\\
& & called. String tokens are [=|!|\#|+var|-var]*\\
& & {[}default=''''{]}\\
\emph{-{}-reverse-depend, -r} &-& Reverse dependency info on in/dependent\\
& & variables\\
\emph{-{}-clear-depend, -e} &-& Clear dependency information on
variables\\
\emph{-{}-heuristic <string>,}  &-&
Choose heuristic j=LSGB, l=Chaff-like\\
\emph{~~~~~~~-H <string>} & & lemma, i=Interactive {[}default=\char`\"{}j\char`\"{}{]}\\
...
to\\
& & search for.{[}default=1{]}\\
\emph{-{}-sbj <number>} &-& Super backjumping {[}default=0{]}\\
\emph{-{}-max-vbles-per-smurf <number>,} &-&
Set the maximum number variables per\\
...
Number of backtracks to start the trace\\
...
\vspace*{-3mm}\\
\indent{\tt \$ sbsat -{}-ini <path-to-initialization-file>}

\noindent
If only a filename is specified instead of a path, SBSAT looks in the
current directory for the file.  If the {\tt -{}-ini} switch is not
...
\vspace*{-3mm}\\
\indent{\tt \$ sbsat -{}-create-ini > <user-specified-filename>}

Each line of the initialization file is either whitespace or a command
which sets an internal parameter to a value.  A command consists of a
parameter name on the left, followed by an `{\tt =}' followed by a value
on the right.  A comment is any text on any line following and
including the first occurrence of the `{\tt \#}' character on that line.
The sample initialization file shown in Figure~\ref{init-figure}
presets variables 1-15 at the start of preprocessing, sets the debug
output level to 3, changes the default preprocessing sequence to {\tt
(ExDc)*(ExSt)*(ExPr)*((ExEa)1)*}, and turns on Existentially Quantify
And.  An initialization file only needs to contain the settings the
user wants modified from the default settings.
...
\begin{verbatim}
  ...
  # The preprocessing sequence
  preprocess-sequence="(ExDc)*(ExSt)*(ExPr)*((ExEa)1)*"
  Ea=1
\end{verbatim}
...
Problems to be solved typically are presented to {\tt sbsat} as a file
...
\noindent
and a description is given here.  Other useful formats include XOR,
for handling a rather specific type of problem where each conjunct is
an {\tt xor} of conjuncts, DNF, Tracer (a subset of a language used in
CMU benchmarks), and {\sc Smurf} (a special form directly tied to data
structures).

All inputs formats support up to five different types of statements
...
Lines that get a {\tt \$} number start with {\tt var}, {\tt not}, 
{\tt truth\_table}, {\tt minmax}, {\tt exist}, {\tt universe}, {\tt nor},
{\tt gcf}, {\tt prune}, {\tt restrict}, {\tt nimp}, {\tt nand}, 
{\tt equ}, {\tt imp}, {\tt and}, {\tt xor}, {\tt or}, a single
positive or negative variable, a function previously defined
in a {\tt \#define} statement, or a single number with a {\tt \$} 
in front of it.  Lines that do not get a {\tt \$} number start with
{\tt initialbranch}, {\tt \#define}, {\tt print\_tree}, or {\tt pprint\_tree}.
...
\noindent
A Boolean function may be expressed four ways:\\
\vspace*{-3mm}\\
\indent{\tt VAR <integer>}\\
\indent{\tt <variable>}\\
\indent{\tt <function-identifier> arg1 arg2 ...}\\
\indent{\tt <function-identifier> ( arg1, arg2, ... )}\\
\vspace*{-3mm}\\ 
where {\tt <function-identifier>} is one of {\tt not}, {\tt ite}, {\tt
and}, {\tt nand}, {\tt or}, {\tt nor}, {\tt imp}, {\tt nimp}, {\tt
equ}, {\tt xor}, {\tt and\#}, {\tt or\#}, {\tt nand\#}, {\tt nor\#}
({\tt \#} is a positive integer), and {\tt arg\#} is either a variable
or a Boolean function or a reference to a Boolean function.  The token
{\tt <variable>} is a character string and its use in an input file
creates a simple Boolean function which is identified by that name,
the value of which is the value of the variable named {\tt
<variable>}.  Every Boolean function is implemented as a BDD (see
Section~\ref{bdd-section}) and may have value {\em T}, or {\em F}, or
have no value, depending on its arguments: values of Boolean functions
are given as follows for {\tt VAR} and each {\tt <function-identifier>}.

\indent\textbf{VAR}\\
\vspace*{-3mm}\\
\indent\indent\parbox{110mm}{
Variables may be defined as Boolean functions using {\tt VAR} followed
by a single integer argument, which identifies the created variable.
The use of {\tt VAR} is now deprecated and its use is discouraged but
it remains supported (for now) for the benefit of early users.  It is
now assumed that all integers are {\em variable names} unless preceded
by a '\$'. All integers preceded by a '\$' refer to functions or
...
complement of its single argument, if it has a value.  Otherwise this
function has no value.  A variable may also be complemented by
using '{\tt -}' in place of {\tt NOT}.  Examples are:}\\
\vspace*{0mm}\\
...
\vspace*{-3mm}\\
\indent\indent\parbox{110mm}{
The ternary Boolean function {\tt ITE} has the value of its second
argument if that argument has a value and its first argument has value
{\em T}, has the value of its third argument if that argument has
a value and its first argument has value {\em F}, and has no
value otherwise.  Examples of this function are the following:}\\
\vspace*{0mm}\\
...
arguments have value {\em T}, has value {\em F} if at least one of its
arguments has value {\em F}, and has no value otherwise.
...
arguments have value {\em T}, has value {\em T} if at least one of its
arguments has value {\em F}, and has no value otherwise.
Examples are the following:}\\
\vspace*{0mm}\\
...
has value {\em T}, has value {\em F} if both its arguments have value
{\em F}, and has no value otherwise. Examples are:}\\
\vspace*{0mm}\\
\indent\indent\indent {\tt or(x3, x4)}\\
\indent\indent\indent {\tt or x3 x4}
...
arguments has value {\em T}, has value {\em T} if both arguments have
value {\em F}, and has no value otherwise.  Examples are:}\\
\vspace*{0mm}\\
\indent\indent\indent {\tt nor(x3, x4)}\\
\indent\indent\indent {\tt nor x3 x4}

\indent\textbf{EQU}\\
\vspace*{-3mm}\\
\indent\indent\parbox{110mm}{
This binary Boolean function has value {\em T} if both its arguments
have been assigned the same value, has value {\em F} if both its
arguments have been assigned different values, and has no
value if at least one of its arguments has not been assigned a
value.  In the following examples {\tt equ} has value {\em T}
if and only if variable {\tt x4} is assigned value {\em T}.}\\
\vspace*{0mm}\\
...
been assigned opposite values, has value {\em F} if its two arguments
have been assigned the same value, and has no value otherwise.}\\
\vspace*{0mm}\\
...
\vspace*{-3mm}\\ 
\indent\indent\parbox{110mm}{ 
This binary Boolean function has value {\em T} if its first argument has
value {\em T} or has value {\em F} and its second argument has value
{\em F}, has value {\em F} if its first argument has value {\em F}
and second value {\em T}, and has no value otherwise. Examples are:}\\
\vspace*{0mm}\\
...
\vspace*{-3mm}\\
\indent\indent\parbox{110mm}{
This binary Boolean function has value {\em F} if its first argument has
value {\em T} or has value {\em F} and its second argument has value
{\em F}, has value {\em T} if its first argument has value {\em F}
and second value {\em T}, and has no value otherwise.  Examples are:}\\
\vspace*{0mm}\\
\indent\indent\indent {\tt xor(x3, x4)}\\
\indent\indent\indent {\tt xor x3 x4}
...
The functions of this section are extensions to some of the binary
...
is immediately followed by a number indicating the number of arguments
...
{\em T}, has value {\em F} if at least one argument has value {\em F},
and has no value otherwise.  Examples of its use are as
follows:}\\
\vspace*{-0mm}\\
...
{\em T}, has value {\em T} if at least one argument has value {\em F}, 
and has no value otherwise.  Examples of its use are as follows:}\\
\vspace*{0mm}\\
\indent\indent\indent{\tt nand4(x1, x2, x3, x4)}\\
\indent\indent\indent{\tt nand5(-x1,}\\
\indent\indent\indent\indent{\tt nand3(x2, x3, \$4),}\\
\indent\indent\indent\indent{\tt x5, \$6, x7)}
...
\vspace*{-3mm}\\
\indent\indent\parbox{110mm}{
This Boolean function has value {\em T} if all its arguments have
values and those values are all the same, has value {\em F} if 
at least one pair of arguments has different values, and otherwise 
has no value.}\\
...
\indent\indent\parbox{110mm}{
This Boolean function has value {\em T} if all its arguments have
values and an odd number of those values are {\em T}, has value
{\em F} if all its arguments have values and an even number 
of those values are {\em T}, and has no value otherwise.}\\
...
\vspace*{-2mm}\\ 
\indent\indent\parbox{110mm}{ 
where {\tt <cofactor>} and {\tt <function>} are both Boolean functions
or variables, and {\tt <cofactor>} is the complement of the ``don't
care set'' of {\tt <function>}.  The Boolean function returned by {\tt
gcf} is the Generalized Co-factor of {\tt <function>} with respect to
{\tt <cofactor>}.  See Section~\ref{gcf-section} for a description of
generalized cofactor and when it might be useful.}\\
...
\indent\textbf{RESTRICT}\\
\vspace*{-3mm}\\
\indent\indent\parbox{110mm}{
The syntax for this manipulator is}\\
\vspace*{-3mm}\\
\indent\indent\indent{\tt restrict <function>\_1 <function>\_2}\\
\indent\indent\indent{\tt restrict( <function>\_1, <function>\_2)}\\
\vspace*{-2mm}\\
\indent\indent\parbox{110mm}{
where {\tt <function>\_1} and {\tt <function>\_2} are both Boolean
...
satisfies the function returned by {\tt restrict}.  To find out more
about restrict see Section~\ref{branch-prune-section}.}\\
...
That is, let $f_{T}$ be {\tt <function>} with {\tt <variable>} set to
{\em T} and let $f_{F}$ be {\tt <function>} with {\tt <variable>} set
to {\em F}; then the Boolean function returned is $f_{T}\vee f_{F}$.
...
where {\tt <function} is a Boolean function and {\tt <variable>} is a
Boolean variable that {\tt <function>} depends on.  This manipulator
resolves away occurrences of {\tt <variable>} in {\tt <function>}.
That is, consider {\tt <function>} to be expressed as a BDD, search
for all occurrences of {\tt <variable>} in the BDD, and replace those
nodes with the logical ``and'' of their substructures.  For more
information about universe see Section~\ref{universe-section}.}\\
...
\indent\indent\indent
{\tt minmax (<\#args>, <min\_\#>, <max\_\#>, <comma\_separated\_var\_list>)}\\
\vspace*{0mm}\\
\indent\indent\parbox{110mm}{
where the first three arguments are positive integers.  This function
returns {\em T} if at least {\tt <min\_\#>} and at most {\tt
<max\_\#>} of the variables in the {\tt <...\_var\_list>} have value
{\em T}, returns {\em F} if more than {\tt <max\_\#>} or less than
{\tt <min\_\#>} of the variables in the {\tt <...\_var\_list>} have 
value {\em T}, returns no value otherwise.  The number {\tt <\#args>}
is the number of variables in the {\tt <...\_var\_list>}.  The
following is an example,}\\
\vspace*{0mm}\\
%\indent\indent\indent {\tt pprint\_tree( minmax(4, 1, 3, 13, 12, 11,
%10))}\\
...
\vspace*{-3mm}\\ 
\indent\indent\parbox{110mm}{ 
The directive {\tt INITIAL\_BRANCH} may be invoked to request that
certain variables be assigned values before any others when searching.
The directive takes as argument a comma separated list of variables,
wrapped between parentheses.  The list may be of any length and may be
continued over many lines.  This directive is intended to be used when
some key domain-specific information suggesting a particular search
order has been revealed.  An example of this directive is the
following:}\\
\vspace*{0mm}\\
\indent {\tt ~~~initial\_branch (x1, x39, x5, x4, x24, x3)}\\
\vspace*{0mm}\\
\indent\indent\parbox{110mm}{ 
This marks variables $x_1$, $x_{39}$, $x_5$, $x_4$, $x_{24}$, $x_3$ to
be assigned values first.  The order in which these variables are
selected for search completely depends on the search heuristic
employed: all that {\tt initial\_branch} does is prevent variables
which are not listed from being selected for assignment during search
until all the listed variables have values.  It is possible that a
non-listed variable is inference, however.  This directive is the only
one which requires parenthesis around its argument list.  Wildcards
are allowed: '{\tt *}' matches any combination of alphabetic, numeric,
and underscore characters.  Thus, {\tt initial\_branch(x3*3)} means
branch on all variables beginning with {\tt x3} and ending with {\tt
3}.  The order in which wild card variables are searched is
determined}\\ 

\indent\indent\parbox{110mm}{
by the search heuristic.  There can be only one {\tt initial\_branch}
directive in a file and it must be placed right after the header.}\\
...
\vspace*{0mm}\\
\indent\indent\parbox{110mm}{The line}\\
\vspace*{-2mm}\\
\indent\indent\indent{\tt print\_tree(minmax(4, 1, 3, x13, x12, x11, x10))}\\
\vspace*{0mm}\\
\indent\indent\parbox{110mm}{
prints the following to standard output:\\
...
structure section.  Secification of these sections is shown in
...
\vspace*{-3mm}\\ 
where {\tt <function-identifier>} is one of {\tt and}, {\tt or}, {\tt
nand}, {\tt nor}, {\tt xor}, {\tt xnor}, {\tt not}, and {\tt ite}.
The variable lists of all but {\tt ite} and {\tt not} can be
arbitrarily long.  The argument lists for {\tt ite} and {\tt not} must
have exactly three and exactly one argument(s), respectively.  Observe
there is no nesting of functions as in the case of the canonical form.
Instead, an equality is defined and the leftside (temporary) variable,
not appearing in either input or output section, is used as argument
in other functions.  Unlike the case for the canonical form, it is
permissable to reference a variable that appears for the first time
further ahead in the file.  A solution is an assignment to input
variables which causes all functions (statements) to have value {\em
T}.

\subsection{{\sc Smurf} format}\label{ref-smurf-section}

This input format is intended for low-level truth table input.
Variables are represented as non-negative integers.  Any set of
...
which satisfies the properties $index(v) < \min\{index(true(v)),
...
\subsection{Existentially quantify and}\label{existential-all-section}
...
\subsection{Restrict}\label{branch-prune-section}

Restrict is an operation on two BDDs.  The intention of restrict
is to remove paths from one BDD which are made irrelevant by the
other BDD.  The following specifies how this is done in pseudo-C++ style:\\
\vspace*{-3mm}\\
{\footnotesize
\indent{\tt BDD Restrict (BDD {\em f}, BDD {\em c}) \{}\\
...
Restrict({\em f}, ExQuant({\em c}, $x_c$));}\\
\indent\indent{\tt if (Reduce$_F$($x_f$, c) == {\em F}) return
Restrict(Reduce$_T$($x_f$, {\em f}), Reduce$_T$($x_f$, {\em c}));}\\
\indent\indent{\tt if (Reduce$_T$($x_f$, c) == {\em F}) return
Restrict(Reduce$_F$($x_f$, {\em f}), Reduce$_F$($x_f$, {\em c}));}\\
\indent\indent{\tt let $h_{f_T}$ = Restrict(Reduce$_T$($x_f$, f),
  Reduce$_T$($x_f$, c));  // $h_{f_T}$ is a BDD}\\
\indent\indent{\tt let $h_{f_F}$ = Restrict(Reduce$_F$($x_f$, f),
...
\indent{\tt \}}\\
\vspace*{-3mm}\\}
The procedure {\em Restrict} takes two BDDs which are top-level functions
...
\caption{Example of {\bf restrict}.  Procedure Restrict is applied to
...
\caption{Restrict on one variable ordering produces no result.}
...
restricting the two functions in Figure~\ref{prune-2-figure}.}
\label{prune-3-figure}
\end{figure}

Restrict can reveal inferences but this depends on the variable
ordering.  Figure~\ref{prune-2-figure} shows Restrict applied to two
BDDs with no result.  BDDs representing the same two functions but
under a different variable ordering are Restricted in
Figure~\ref{prune-3-figure} revealing the inference $x_3=F$.

Restrict is similar to a procedure called generalized cofactor
or constrain (see Section~\ref{gcf-section} for a description).  Both
Restrict$(f,c)$ and gcf$(f,c)$ agree with $f$ {\em on interpretations
...
considered ``non-logical.''  Restrict is implemented in SBSAT because the
BDDs produced from it tend to be smaller.  In any case, unlike for
gcf, BDDs can never gain in size using Restrict.

There appear to be two gains to using Restrict.  First, it can make
...
(see below), restrict can still cause the number of choicepoints to
increase.  Both these issues are related: restrict can spread an
inference that is evident in one BDD over multiple BDDs (see
Figure~\ref{restrict-1-figure} for an example).

\begin{figure}
\hspace*{6mm}\epsfig{figure=Fig/restrict.eps, width=4.5in}
\caption{Example of {\bf restrict} spreading an inference from one BDD
to another.  If $x_2$ is assigned 0 in $f$ then $x_4=0$ and $x_3=0$
are inferred.  After applying {\tt restrict} to $f$ and $c$ and
replacing $f$ with $f^\prime$, to get the inference $x_3=0$ from
the choice $x_2=0$ visit $c$ to get $x_4=0$ and then $f^\prime$ to
get $x_3=0$.  Thus, restrict can increase work if not used properly.
In this case, restricting in the reverse direction leads to a better
result.}
\label{restrict-1-figure} 
\end{figure}
...
by Restrict due to its sensitivity to variable ordering.
...
substitution} to reduce BDD size.  However, unlike {\tt restrict},
...
somehow reduced compared to $f$ as is the case for {\tt restrict}.
Unlike {\tt restrict}, the following is true as well:\\
...
instance has no solution.  However, unlike {\tt restrict}, generalized
...
\subsection{Clustering: CNF}\label{clustering-section}
...
\subsection{Inference}\label{inference-section}
...
constrain (branch-prune) to simplify the collection.  To ``project''
...
\subsection{WMV}\label{wmv-section}
