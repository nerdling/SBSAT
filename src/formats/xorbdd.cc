...
//		dX_printf(4, "x[%s(%d)] + 1", s_name(xdd->variable), xdd->variable);
//		d2_printf1(")");

		d2e_printf2("-x[%s]", s_name(xdd->variable));
		dX_printf(3, "-x[%d]", xdd->variable);
		dX_printf(4, "-x[%s(%d)]", s_name(xdd->variable), xdd->variable);
...
void print_xor_of_ands(xor_of_ands *top_xor) {
	for(int x = 0; x < top_xor->curr_length-1; x++) {
		if(top_xor->vars_in_and[x] == -1) { // This should never happen
			...
			dX_printf(4, "x[%s(%d)]*", s_name(top_xor->vars_in_and[x]), top_xor->vars_in_and[x]);
		}
	}
	if(top_xor->vars_in_and[top_xor->curr_length-1] == -1) fprintf(foutputfile, "1");
		...
		dX_printf(4, "x[%s(%d)]", s_name(top_xor->vars_in_and[top_xor->curr_length-1]), top_xor->vars_in_and[top_xor->curr_length-1]);
	}
	for(xor_of_ands *tmp = top_xor->next_and; tmp!=NULL; tmp = tmp->next_and) {
		...
		for(int x = 0; x < tmp->curr_length-1; x++) {
			if(tmp->vars_in_and[x] == -1) { // This should never happen
				...
				dX_printf(4, "x[%s(%d)]*", s_name(tmp->vars_in_and[x]), tmp->vars_in_and[x]);
			}
		}
		if(tmp->vars_in_and[tmp->curr_length-1] == -1) { 
			...
			dX_printf(4, "x[%s(%d)]", s_name(tmp->vars_in_and[tmp->curr_length-1]), tmp->vars_in_and[tmp->curr_length-1]);
		}
	}
}
...
//		functionType[nmbrFunctions] = XDD;
      ...
      dX_printf(4, "BDD $%d: ", nmbrFunctions);
      if (DEBUG_LVL >= 4) {
        printBDDfile(functions[nmbrFunctions], stddbg);
      }
      dX_printf(4, "\n");
      nmbrFunctions++;
