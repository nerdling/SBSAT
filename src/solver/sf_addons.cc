...
AssertNullAddons(BDDNodeStruct *pFunc)
{
  ...
  if (!IS_TRUE_FALSE(pFunc))
    {
      AssertNullAddons(pFunc->thenCase);
      AssertNullAddons(pFunc->elseCase);
    }
}


ITE_INLINE void
InitializeAddons(BDDNodeStruct *pFunc)
{
   if (pFunc->addons == NULL)
   {
      ...
      if (!IS_TRUE_FALSE(pFunc))
      {
         InitializeAddons(pFunc->thenCase);
         InitializeAddons(pFunc->elseCase);
      }
   }
}

ITE_INLINE void
FreeAddons(BDDNodeStruct *pFunc)
{
   if (pFunc->addons != NULL && !IS_TRUE_FALSE(pFunc))
   {
      if (!IS_TRUE_FALSE(pFunc))
      {
