...
ITE_INLINE int
NullHeuristic(int *pnBranchAtom, int *pnBranchValue)
	 // Return the first uninstantiated variable
{
  int nBranchAtom = 1;

  while (arrSolution[nBranchAtom] != BOOL_UNKNOWN)
    {
      nBranchAtom++;
      assert(nBranchAtom <= gnMaxVbleIndex);
    }


  *pnBranchAtom = nBranchAtom;
