...
LSGBXorGetHeurScores(int nFnId)
{
   ...
   //if (arrSolverFunctions[nFnId].fn_xor.LinkedSmurfs == -1) {
      ...
      fScore, fScore);
   /*} else {
      SmurfState *pState = arrRegSmurfInitialStates[arrSolverFunctions[nFnId].fn_xor.LinkedSmurfs];
...
LSGBXorUpdateFunctionInfEnd(int nFnId)
{
   ...
   /*} else {
      ...
      SmurfState *pState = arrPrevStates[arrSolverFunctions[nFnId].fn_xor.LinkedSmurfs];
      if (pState == pTrueSmurfState) {
