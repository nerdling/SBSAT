...
double *arrMiniSatHeurScoresPos;
double *arrMiniSatHeurScoresNeg;
...
ITE_INLINE
void
InitMiniSatHeurArrays (int nMaxVbleIndex)
{
   arrMiniSatHeurScoresPos = (double*)ite_calloc(nMaxVbleIndex+1, sizeof(double), 9, "arrMiniSatHeurScoresPos");
   arrMiniSatHeurScoresNeg = (double*)ite_calloc(nMaxVbleIndex+1, sizeof(double), 9, "arrMiniSatHeurScoresNeg");
   ...
   {
      arrMiniSatHeurScoresPos[i] = 0.0;
      arrMiniSatHeurScoresNeg[i] = 0.0;
...
{
   ite_free((void**)&arrMiniSatHeurScoresPos);
   ite_free((void**)&arrMiniSatHeurScoresNeg);
...
//
ITE_INLINE void
AddMiniSatSpaceHeuristicInfluence(int *arr, int num)
{
...
//      fprintf(stderr, "lit=%d, ", nLiteral);
      if (nLiteral >= 0)
      {
         //arrMiniSatHeurScoresPos[nLiteral] += fWght;
         arrLemmaVbleCountsPos[nLiteral]++;
      }
      else
      {
         //arrMiniSatHeurScoresNeg[-nLiteral] += fWght;
...
ITE_INLINE void
AddMiniSatBlockHeuristicInfluence(LemmaBlock *pLemmaBlock)
{
...
ITE_INLINE void
AddMiniSatHeuristicInfluence(LemmaInfoStruct * pLemmaInfo)
{
   LemmaBlock *pLemmaBlock = pLemmaInfo->pLemma;
   AddMiniSatBlockHeuristicInfluence(pLemmaBlock);
}


ITE_INLINE void
RemoveMiniSatHeuristicInfluence (LemmaInfoStruct * pLemmaInfo)
   // Updates : arrMiniSatHeurScoresPos[], arrMiniSatHeurScoresNeg[].
