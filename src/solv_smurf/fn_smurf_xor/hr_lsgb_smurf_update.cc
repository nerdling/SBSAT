...
{
   LSGBSmurfSetHeurScores(nFnId, arrSolverFunctions[nFnId].fn_smurf.pInitialState, 0);
   ...
   int j=0;
   int k;
   //if (arrSmurfChain[nFnId].specfn == -1) {
   ...
   /*} else {
      int n = arrNumRHSUnknowns[arrSmurfChain[nFnId].specfn];
      for (k = 0; k < pState->vbles.nNumElts; k++) {
         int nVble = arrElts[k];
...
LSGBSmurfUpdateFunctionInfEnd(int nFnId) {
   ...
   /*} else {
      pSpecialFunc = arrSpecialFuncs + arrSmurfChain[nFnId].specfn;
      nPrevNumRHSUnknowns = arrPrevNumRHSUnknowns[arrSmurfChain[nFnId].specfn];
