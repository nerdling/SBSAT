...
int GBReduceXors(){
  ...
  if(xor_indxs.size() > 0){
    ...
    for(vector<int>::iterator it = xor_indxs.begin(); it != xor_indxs.end(); it++){
      ...
      ss << "\"";
      var = build_algebraic_func(xdd,ss);
      if(var > maxvar){
	maxvar = var;
      }
      ss << "\" ";
    ...
    //call each time. (Or call python from C as per python docs)
    ...
    stringstream ss2;
    ss2 << "sage /home/tj/cryptosat/SBSAT/scripts/gb.sage " << maxvar + 1 << " " << ss.str();
    //    cout << "ss2 is " << ss2.str() << endl;
    
    string sagestr = ss2.str();
    char * sagecmd = new char[sagestr.size()];
    strcpy(sagecmd,ss2.str().c_str());
    cout << "Sage command is: " << sagecmd << endl; 
    FILE * sagepipe;
  
    sagepipe = popen(sagecmd,"r");
  
    if(sagepipe == NULL){
      //cerr << "Error: couldn't open sage pipe" << endl;
      return -1; // check this return val...
    }
...
int build_algebraic_func(BDDNode* xdd, stringstream& ss){
  ...
  int var = xdd->variable;
  if(var > maxvar){
    maxvar = var;
  }
  if(!IS_TRUE_FALSE(xdd->thenCase)){
    ss << "x" << var << "*(";
    var = build_algebraic_func(xdd->thenCase,ss);
    if(var > maxvar){
      maxvar = var;
    }
    ss << ")";
  ...
  if(xdd->elseCase == true_ptr){
    ...
    ss << "+";
    var = build_algebraic_func(xdd->elseCase,ss);
    if(var > maxvar){
      maxvar = var;
    }
  }
  //  cout << "returning max var: " << maxvar << endl;
  return maxvar;
}
...
void _Dd_Support(int *y, int *max, int **tempint, DdNode * func);

void Dd_Support(int *y, int *max, int **tempint, DdNode * func) {
  *y=0;
  _Dd_Support(y, max, tempint, Cudd_Regular(func));
  for (int i = 0;i<*y;i++) {
    // clear the flag
    sym_reset_flag((*tempint)[i]);
  }
  ddClearFlag(Cudd_Regular(func));
}

void _Dd_Support(int *y, int *max, int **tempint, DdNode * func) {
  if(cuddIsConstant(func) || Cudd_IsComplement(func->next)) {
    return;
  }

  if (sym_is_flag(func->index) == 0) {
    if (*y >= *max) {
      *tempint = (int*)ite_recalloc(*(void**)tempint, *max, *max+100, sizeof(int), 9, "tempint");
      *max += 100;
    }
    (*tempint)[*y] = func->index;
    sym_set_flag(func->index);
    (*y)++;
  }
  _Dd_Support (y, max, tempint, cuddT(func));
  _Dd_Support (y, max, tempint, cuddE(func));
